   ...
   | Ast.Binop (e1, op, e2) ->
     ...
     if op=Ast.Add && e1Type="Text" && e2Type="Text" then
		let funRecord=getExistingFunOrNew ("strAdd") (Cast.PointerType(Cast.Char)) [Cast.FormalDecl(Cast.PointerType(Cast.Char),"a");Cast.FormalDecl(Cast.PointerType(Cast.Char),"b")] cenv.funs in
  		  ...
  		  (* create the function *)
  		  let globalDecl= Cast.OneDArrDecl(Cast.BasicType(Cast.Char),"buffer",Cast.Literal(256)) in
	  	  let stmts = [
				Cast.Expr(Cast.Call("strcpy",[Cast.Id("buffer");Cast.Id("a")]));
				Cast.Expr(Cast.Call("strcat",[Cast.Id("buffer");Cast.Id("b")]));
				Cast.Return(Cast.Id("buffer"))
	  	  ] in
          let newFunRecord = { returnType = funRecord.returnType; fname = funRecord.fname; formals = funRecord.formals; 
          locals=funRecord.locals ; body= stmts } in
          let cenv={types=cenv.types; globals=cenv.globals @ [globalDecl]; funs=newFunRecord :: cenv.funs} in
          curFun, cenv, env, "Text", [Cast.Call("strAdd", [List.hd e1CExpr; List.hd e2CExpr])]         
  	      else
          curFun, cenv, env, "Text", [Cast.Call("strAdd", [List.hd e1CExpr; List.hd e2CExpr])] 
     else
     let retType, castOp=
     match op with
        Ast.Add -> 
          if e1Type = "Numeric" && e2Type = "Numeric" then
                "Numeric", Cast.Add
          else raise (Failure ("Error: Add expression should take numeric or text type expression "))   
...
let createCSyntax curFun cenv env variable containerExpr widgetType valueList=
  if StringMap.mem variable env then
 		 raise (Failure ("Error: Duplicate variable " ^ variable))
  else	
  let containerVar=
  match containerExpr with 
  Ast.Var(x) -> x
  | _ -> raise (Failure ("Error: only support create widget from container Ast.Var "))
  in  	 
...
let evalShow curFun cenv env callerExpr = 
  let variable=
  match callerExpr with 
  Ast.Var(x) -> x
  | _ -> raise (Failure ("Error: only support show on Ast.Var "))
  in
  let vType = getVarTypeOrFail variable env in
  let stmts =
  match vType with 
...
(*define non-Gtk variable*)
 ...
 | Ast.Show (callerExpr) ->
   let curFun, cenv, env= evalShow curFun cenv env callerExpr in
    curFun, cenv, env, String "Show"     
 | Ast.Action (callerExpr, actionType, expr) ->
	let variable=
	  match callerExpr with 
	  Ast.Var(x) -> x
	  | _ -> raise (Failure ("Error: only support register action on Ast.Var "))
	in
    let vType = getVarTypeOrFail variable env in
    (* callerType: Button, ... ; callbackFunName: button1; callerCExpr: first argument in g_signal_connect function  *)
    let callerType, callbackFunName, callerCExprs = vType, variable, [Cast.Id (variable)]
    in   
  ...
  | Ast.Create (variable, containerExpr, widgetType, valueList) ->  
       let curFun, cenv, env, vdecls, stmts= createCSyntax curFun cenv env variable containerExpr widgetType valueList in       
         let lfdecl = curFun in
         let nfdecl = { returnType = lfdecl.returnType; fname = lfdecl.fname; formals = lfdecl.formals; 
...
(* input: pear pre-process type return: c pre-process type *)
let evalPreProc preProc=match preProc with
    Ast.IncludeSys(filename) -> Cast.IncludeSys(filename)
  | Ast.IncludeUser(filename) -> Cast.IncludeUser(filename)

let rec evalPreProcList = function
  [] -> []
  | hd::tl -> (evalPreProc hd) :: (evalPreProcList tl)

let _ =
  let lexbuf = Lexing.from_channel stdin in
  let program = Parser.program Scanner.token lexbuf in
  let _, cenv, evars, result = exec vars program.progBody in
  ...
  let nfuns = updateCFuns nfdecl [] cenv.funs in  
  let cPreProcess=evalPreProcList program.preProcList in
  let cProgBody={types=cenv.types; globals=cenv.globals; funs=nfuns} in
  let listing = Cast.string_of_program ( {preProc=cPreProcess; pBody=cProgBody} ) in
