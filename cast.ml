...
let rec string_of_preProcess = function
     IncludeSys(filename) -> "#include <" ^ filename ^ ">\n"
   | IncludeUser(filename) -> "#include \"" ^ filename ^ "\"\n" 

let string_of_progBody progBody =
  String.concat "" (List.map string_of_sdef progBody.types) ^ "\n" ^
  String.concat "" (List.map string_of_vdecl progBody.globals) ^ "\n" ^
  String.concat "\n" (List.map string_of_fdecl progBody.funs)
  
let string_of_program program =
  String.concat "" (List.map string_of_preProcess program.preProc) ^ "\n" ^
  string_of_progBody program.pBody
